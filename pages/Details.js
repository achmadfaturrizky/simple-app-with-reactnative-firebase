...
class Details extends Component {
    ...
    const foto = navigation.getParam('foto', 'foto');
    const title = navigation.getParam('title', 'title');
    const kategori = navigation.getParam('kategori', 'kategori');
    const deskripsi = navigation.getParam('deskripsi', 'deskripsi');
    const kapasitas = navigation.getParam('kapasitas', 'kapasitas');
    const lokasi  = navigation.getParam('lokasi', 'lokasi');
        return(
                ...
                <View>
                 ...
                 >
                <ImageBackground
                                style={{width: '100%', top: -30, height: 300, resizeMode: 'cover'}}
                                source={{uri: foto}}
                            >
              ...
              onPress={() => this.props.navigation.goBack()}
                /><Card>
                            ...
                            <Text style={{fontWeight: 'bold', top: -50, color:'#000'}}>Waktu & Tempat</Text>
                           
                            
                             
                            <Text style={styles.right}>
                            <Icon type="AntDesign" style={{color:'#FF553F', fontSize:15}} name="enviromento" />
                             {lokasi}</Text>
                            <Text style={styles.right}><Icon type="AntDesign" style={{color:'#FF553F', fontSize:15}} name="wallet" />
                             Rp.{harga}</Text>
                            <Text style={styles.right}><Icon type="AntDesign" style={{color:'#FF553F', fontSize:15}} name="team" />
                             {kapasitas}</Text>
                           <Text style={{fontWeight: 'bold', top: -30, color:'#000'}}>Deskripsi</Text>
            ...
            <Text style={styles.isi}>{deskripsi}</Text>
            
            </ReadMore>
            <Right>
            <Button onPress={() => navigate('Test', {harga: harga, title: title, kategori: kategori})}
